    ...
    module Debugger
      class Node < Struct.new(:captured_node, :task, :activity, :compile_id, :compile_path, :runtime_id, :runtime_path, :label, :data, :snapshot_before, :snapshot_after, :level, keyword_init: true)
        ...
        def self.build(tree, enumerable_tree, node_options: {}, normalizer: Debugger::Normalizer::PIPELINES.last, **options_for_nodes)
          parent_map = Developer::Trace::Tree::ParentMap.build(tree).to_h # DISCUSS: can we use {enumerable_tree} for {ParentMap}?
      ...
      def self.trace_for_stack(stack, **options_for_debugger_nodes)
        tree, processed = Developer::Trace.Tree(stack.to_a)

        enumerable_tree = Developer::Trace::Tree.Enumerable(tree)
