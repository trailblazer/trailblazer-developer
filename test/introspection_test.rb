...
class IntrospectionTest < Minitest::Spec
  ...
  describe "Introspect::Graph" do
    let(:graph) { Dev::Introspect::Graph(nested_activity) }
    ...
    describe "#find" do
      ...
      it { node[:outgoings].inspect.must_equal(%{[#<struct Trailblazer::Developer::Introspect::Graph::Outgoing output=#<struct Trailblazer::Activity::Output signal=Trailblazer::Activity::Right, semantic=:success>, task=#{bc.inspect}>]}) }
